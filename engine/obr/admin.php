...
if($_POST['action'] == "delete_item_roulette")
{
	...
	if(!empty($id))
	{
		...
		else message('warning','Системна Помилка!',"Нам не вдалось видалити предмет, перевірте наявність таблиці - ucp_item_roulette", "/admin/roulette");
	}
...
if($_POST['action'] == "save_category_roulette")
{
	...
	if(!empty($name)  && !empty($id))
	{
		...
		else message('warning','Системна Помилка!',"Нам не вдалося зберегти категорію, перевірте наявність таблиці - ucp_category_roulette", "/admin/roulette");
	}
...
if($_POST['action'] == "delete_category_roulette")
{
	...
	if(!empty($id))
	{
		...
		$result = $db->query($sql, PDO::FETCH_ASSOC);
		if($result) message('success','Успіх!',"Ви успішно видалили категорію!", "/admin/roulette");
		else message('warning','Системна Помилка!',"Нам не вдалося видалити категорію перевірте наявність таблиці - ucp_category_roulette", "/admin/roulette");
	}
...
if($_POST['action'] == "create_category_roulette")
{
	...
	if(!empty($name))
	{
		...
		$result = $db->query($sql, PDO::FETCH_ASSOC);
		if($result) message('success','Успіх!',"Ви успішно створили категорію!", "/admin/roulette");
		else message('warning','Системна Помилка!',"Нам не вдалось створити категорію, перевірте наявність таблиці - ucp_category_roulette", "/admin/roulette");
	}
	else message('warning','Помилка!',"Заповніть поле");	
...
if($_POST['action'] == "create_news")
{
	...
	if(!empty($n_images) && !empty($n_text) && !empty($n_url) && !empty($n_title))
	{
		...
		else message('warning','Системна Помилка!',"Нам не вдалось створити новину, перевірте наявність таблиці - ucp_news", "/admin/news");
	}
	else message('warning','Помилка!',"Заповніть поле");	
...
if($_POST['action'] == "create_item_roulette")
{
	...
	if(!empty($i_name) && !empty($i_images) && !empty($i_category) && !empty($i_change) && !empty($i_start_rand) && !empty($i_end_rand))
	{
		...
		$result = $db->query($sql, PDO::FETCH_ASSOC);
		if($result) message('success','Успіх!',"Ви успішно додали предмет в рулетку!", "/admin/roulette");
		else message('warning','Системна Помилка!',"Нам не вдалось стоврити предмет, перевірте наявність таблиці - ucp_item_roulette", "/admin/roulette");
	}
	else message('warning','Помилка!',"Заповніть поле");
...
if($_POST['action'] == "save_item_roulette")
{
	...
	if(!empty($i_name) && !empty($i_images) && !empty($i_category) && !empty($i_change) && !empty($i_start_rand) && !empty($i_end_rand) && !empty($id))
	{
		...
		$result = $db->query($sql, PDO::FETCH_ASSOC);
		if($result) message('success','Успіх!',"Ви успішно зберегли предмет!", "/admin/roulette");
		else message('warning','Системна Помилка!',"Нам не вдалось зберегти предмет, перевірте наявність таблиці - ucp_item_roulette", "/admin/roulette");
	}
	else message('warning','Помилка!',"Заповніть поле");
...
if($_POST['action'] == "update_table_setting")
{
	...
	if(!empty($value))
	{
		...
		if($result) message('success','Успіх!',"Ви успішно оновили налаштування!");
		else message('warning','Системна Помилка!',"Нам не вдалось зберегти інформацію, перевірте наявність таблиці - ucp_table_settings", "/admin/table");
	}
	else message('warning','Помилка!',"Заповніть поле");	
...
if($_POST['action'] == "save_settings_ucp")
{
	...
	if(!empty($s_title) && !empty($s_logo) && !empty($s_favicon) && !empty($s_donate_cost))
	{
		...
		$statement->execute();
		if($statement->rowCount()) message('success','Успіх!',"Ви успішно оновили інформацію! Зарас буде оновлено", "/admin/");
		else message('warning','Системна Помилка!',"Нам не вдалось зберегти інформацію, перевірте наявність таблиці - ucp_settings", "/admin/");
